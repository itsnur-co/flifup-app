/**
 * Task Category Filter Component
...
interface CategoryWithCount extends TaskCategory {
  count?: number;
}

export const TaskCategoryFilter: React.FC<CategoryFilterProps> = ({
  categories,
  selectedCategoryId,
  onSelectCategory,
  onAddCategory,
  ...
  const allCategories: (CategoryWithCount | { id: null; name: string; count: number })[] = [
    { id: null, name: "All Tasks", count: totalCount },
    ...categories.map((cat) => ({
      ...cat,
      count: cat._count?.tasks || 0,
    })),
  ];
